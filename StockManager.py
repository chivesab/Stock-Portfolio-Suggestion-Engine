...
def get_latest_price(strategies):
    ...
    for url in url_list:
        ...
        latest_price[item["symbol"]] = {
            "price": item["price"],
            "strategy": get_strategy_by_stock(item["symbol"]),
        }
    return latest_price
...
def allocate_stocks(amount, strategies):
    ...
    latest_price = {
        k: v
        for k, v in sorted(
            latest_price.items(), key=lambda item: item[1]["price"], reverse=True
        )
    }
    stock_amount1, stock_amount2, stock_amount3 = (
        amount * 0.5,
        amount * 0.3,
        amount * 0.2,
    )
    ...
    dates = [
        ...
        for i in range(today.weekday() - 12, today.weekday() - 5)
    ]
    for date in dates:
        temp_dict = {}
    ...
    return {
        ...
        "weekly_trend_by_stock": new_weekly_trend_by_stock,
    }
    ...
    for strategy in strategies:
        for ticker in stocks.get(strategy):
            url = (
                base_url
                + ticker
                + "?timeseries=7&apikey=5e46a24c274b0a3f3a27f46f34bf0bb8"
            )
